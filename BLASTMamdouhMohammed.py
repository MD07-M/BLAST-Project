blosum62 = {
    'A': {'A': 4, 'R': -1, 'N': -2, 'D': -2, 'C': 0, 'Q': -1, 'E': -1, 'G': 0, 'H': -2, 'I': -1, 'L': -1, 'K': -1,
          'M': -1, 'F': -2, 'P': -1, 'S': 1, 'T': 0, 'W': -3, 'Y': -2, 'V': 0, 'B': -2, 'Z': -1, 'X': 0},
    'R': {'A': -1, 'R': 5, 'N': 0, 'D': -2, 'C': -3, 'Q': 1, 'E': 0, 'G': -2, 'H': 0, 'I': -3, 'L': -2, 'K': 2, 'M': -1,
          'F': -3, 'P': -2, 'S': -1, 'T': -1, 'W': -3, 'Y': -2, 'V': -3, 'B': -1, 'Z': 0, 'X': -1},
    'N': {'A': -2, 'R': 0, 'N': 6, 'D': 1, 'C': -3, 'Q': 0, 'E': 0, 'G': 0, 'H': 1, 'I': -3, 'L': -3, 'K': 0, 'M': -2,
          'F': -3, 'P': -2, 'S': 1, 'T': 0, 'W': -4, 'Y': -2, 'V': -3, 'B': 3, 'Z': 0, 'X': -1},
    'D': {'A': -2, 'R': -2, 'N': 1, 'D': 6, 'C': -3, 'Q': 0, 'E': 2, 'G': -1, 'H': -1, 'I': -3, 'L': -4, 'K': -1,
          'M': -3, 'F': -3, 'P': -1, 'S': 0, 'T': -1, 'W': -4, 'Y': -3, 'V': -3, 'B': 4, 'Z': 1, 'X': -1},
    ...
    'Q': {'A': -1, 'R': 1, 'N': 0, 'D': 0, 'C': -3, 'Q': 5, 'E': 2, 'G': -2, 'H': 0, 'I': -3, 'L': -2, 'K': 1, 'M': 0,
          'F': -3, 'P': -1, 'S': 0, 'T': -1, 'W': -2, 'Y': -1, 'V': -2, 'B': 0, 'Z': 3, 'X': -1},
    'E': {'A': -1, 'R': 0, 'N': 0, 'D': 2, 'C': -4, 'Q': 2, 'E': 5, 'G': -2, 'H': 0, 'I': -3, 'L': -3, 'K': 1, 'M': -2,
          'F': -3, 'P': -1, 'S': 0, 'T': -1, 'W': -3, 'Y': -2, 'V': -2, 'B': 1, 'Z': 4, 'X': -1},
        ...
        for k in i:
            ...
            for j in range(len(seeds)):
                if seeds[j] in Db_str:
                    ...
                    Db_id.append(count)
        count += 1
    return hits, hits_score, startindex, endindex, Db_index, Db_id
...
def lowcomplexity(query):
    ...
    while (even > 0) and (odd > 0):
        ...
        for item in query_list:
            query += item;
    print("\n\nQuery after low complexity: ", query,"\n")
    return query


def ExtendQuery(hits, Db, Db_id, query, scores, startindex, endindex, wordlength, Dbindex, HSPthreshold):
    for hit_index in range(len(hits)):
        score = 0
        HSP_threshold = HSPthreshold
        Db_str = ""
        for j in Db[Db_id[hit_index]]:
            Db_str += j;
        # forwad bs
...
Seq = input("Enter Query: ")  # APQGAFGPNTCGVNVDVMMER
...
Hits, Hits_score, startIndex, endIndex, DbIndex, DbID = SeedHit(seeds, Db, scores, index1, index2)
"""for i_neighborhood in range(len(neighborhood)):
    print(neighborhood[i_neighborhood])
...
if len(Hits) == 0:
    ...
    HSP, HSP_score, Database_ID = ExtendQuery(Hits, Db, DbID, Seq, Hits_score, startIndex, endIndex, Wordlength,
                                              DbIndex, Hspthreshold)
    # for i in range(len(HSP)):
    # print("HSP: ",HSP[i],"\t, SCORE: ",str(HSP_score[i]),"\t,ID: ",str((Database_ID[i]+1)))
    l = []
    for i in range(len(HSP)):
        for j in range(len(HSP)):
            if HSP[i] == HSP[j]:
                l.append("HSP: " + HSP[i] + "\t, SCORE: " + str(HSP_score[i]) + "\t,DB-ID: " + str(
                    (Database_ID[i] + 1)) + "\t, DB [i]:" + str(DbIndex[i]))
